...
You can use `workspace-to-monitor-force-assignment` to assign
workspaces to always appear on particular monitors.
Multiple `[[workspace-to-monitor-force-assignment]]` sections can be specified, each with an optional `if.number-of-monitors` condition.
The first section whose condition matches the current number of monitors is used.
If no section matches, the last section is used as a fallback (with a warning).
...
A section without `if.number-of-monitors` matches any number of monitors:
